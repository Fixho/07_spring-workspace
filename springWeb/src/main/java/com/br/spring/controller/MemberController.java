...
public class MemberController {
	...
	public void signin(MemberDto m
		...
		// 아이디와 비번가지고 일치하는 회원 조회
		// 아이디만을 가지고 일치하는 회원 조희(암호화된 비번담겨있음)
	...
	public String modifyProfile(MultipartFile uploadFile
	    ...
	    if(result > 0) {
	    	...
	    	loginUser.setProfileURL(originalProfileURL); // 기존프로필로 다시 변경
	    	return "FAIL";
	    }
	...
	public String resign(String userPwd
		...
		if(bcryptPwdEncoder.matches(userPwd, loginUser.getUserPwd())) { // 비밀번호를 맞게 입력했을 경우
			...
			rdAttributes.addFlashAttribute("alertMsg", "성공적으로 탈퇴되었습니다. 잘가라~");
			session.invalidate();
		}else { // 비밀번호가 틀렸을 경우
